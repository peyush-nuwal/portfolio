    ...
    <div className="absolute top-0 w-full h-full z-20 pointer-events-none">
      {data.map((project, i) => {
        ...
        return (
          <div
            key={i}
            className={`bg-[#ec4e39] h-fit flex justify-between items-center py-4 px-4 lg:px-6  w-full  transition-all duration-500 border-y-[0.1px] border-y-transparent`}
            style={{
          ...
          >
            ...
            <p className="w-[30%] text-base font-bold text-black">
              {description}
            </p>
          </div>
        );
